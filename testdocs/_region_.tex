...
split into two with a connection edge $c$
  ...
  either the emitter or the eye path. This is shown below.
  \begin{align}
    \alpha^{L|E}_i =
    \begin{cases}
...
supported as camera lens are not defined in DTQ. Veach (and many other sources),
leave this in, but it's unnecessarily confusing. For $i = 1$, it is either a
point on the surface of a light, or the origin point on the 'delta eye'. The
connection strategy below describes how to connect the edges of the two paths
  \begin{align}
    c_{s, t} =
    \begin{cases}
...
\end{document}
\message{ !name(test.tex) !offset(-684) }

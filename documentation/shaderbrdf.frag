...
float Rm_glossy_lobe ( float t ) { return 0.80 + 0.20*hash(t, 71341.2342); }
...
float Rm_subsurface  ( float t ) { return hash(t, 2340234.234)*1.0; }
...
vec3 BRDF ( vec3 wi, vec3 wo, vec3 N, int ID, float t ) {
  ...
  // --------- subsurface
  // modified disney retro reflection based off Hanrahan-Grueger BRDF
  float rr_term = (3.0 * (0.5 + m_roughness) * sqr(cosNH));
  vec3 retro_reflection = diffuse * rr_term *
                          (FL + FV + (FL*FV*(rr_term - 1.0)));
  vec3 rd = mix(diffuse, retro_reflection, m_subsurface);
...
vec3 render( in vec3 ro, in vec3 rd, vec2 rand, float time) {
  ...
  for ( int i = 0; i != SAMPLES; ++ i ) {
    ...
    if( mat_col.x>=0.0 ) {
      ...
      col += brdf /(pdf*mult)  * dot(L, N) * Specular_Map(L) ;
    } else {
      col += Diffuse_Map(rd);
    }
  }

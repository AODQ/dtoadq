...
\\
  ...
  \end{align}
    ...
    sources), leave this in, but it's unnecessary and can be confusing. However,
  ...
  \end{align}
    ...
    visible is intuitively obvious as we are already generating these paths.
    Specifically, the definition of $V$ is
  \begin{align}
    V(v \rightarrow \dot{v}) = ||v - \dot{v}|| \leq
    m(v,\:\overrightarrow{v\:\dot{v}})
  \end{align}
    where $m$ is a march through the SDF map. Since the paths are being
    generated using march. A visibility check is only necessary for the
    connection edge
    \begin{align*}
...
\message{ !name(paper.tex) !offset(-368) }
